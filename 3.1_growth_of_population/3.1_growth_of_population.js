...
function nb_year(p0, growth, aug, p) {
    let years = 0;
    while(p0 < p) {
        p0 = p0*(1+growth/100) +aug;
        years++;
    }
    return years;
}

let initial_pop = 1000;
let target_pop = 4000;
let percent = 60;
let aug = 50;

console.log(`At the current rate, the population will surpass ${target_pop} in ${nb_year(initial_pop, percent, aug, target_pop)} years`);
