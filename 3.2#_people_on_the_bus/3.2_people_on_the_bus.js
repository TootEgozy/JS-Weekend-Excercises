...
function lastStopSum (arr) {
    ...
    //map Function
    function callbackMap (subArr) {
        let val = subArr[0] - subArr[1];
        return val;
    }
    //reduce function
     function callbackReduce(acc, val) {
        acc+= val;
        return acc;
    }

    let result = arr.map(callbackMap).reduce(callbackReduce);
    return result;
}

console.log(lastStopSum(stops));